...
class RubiksCube444(RubiksCube):
    ...
    def tsai_phase2_orient_edges_state(self, edges_to_flip):
        ...
        for (index, (x, y)) in enumerate((
            ...
            high_low = tsai_phase2_orient_edges[(x, y, state[x], state[y])]
    ...
    def tsai_phase2_edges_oriented(self):
        ...
        for (index, (x, y)) in enumerate((
            ...
            wing_str1 = "%s%s" % (state[x], state[y])
            wing_str2 = "%s%s" % (state[y], state[x])
            original_high_low_state.append((wing_str1, wing_str2, tsai_phase2_orient_edges[(x, y, state[x], state[y])]))
    ...
    def group_centers_guts(self):
        ...
        # The tsai will solve the centers and pair the edges
                ...
                for front_side_name in ('F', 'R', 'B', 'L', 'U', 'D'):
                    ...
                    if self.rotate_to_side(upper_side_name, front_side_name):
                        ...
                        if min_solution_len is None or solution_len < min_solution_len:
                            ...
                            log.warning("%s: NEW MIN %d (%s, %s)\n\n" % (self, min_solution_len, upper_side_name, front_side_name))
                        else:
            ...
            self.rotate_to_side(min_upper_side_name, min_front_side_name)
            '''

            # TODO:
            # - phase2 needs to be much faster
