...
class LookupTable444FirstFourEdgesStage(LookupTable):
    """
    lookup-table-4x4x4-step301-stage-first-four-edges.txt
    =====================================================
    1 steps has 5 entries (0 percent, 0.00x previous step)
    2 steps has 82 entries (0 percent, 16.40x previous step)
    3 steps has 1238 entries (0 percent, 15.10x previous step)
    4 steps has 14340 entries (1 percent, 11.58x previous step)
    5 steps has 122964 entries (16 percent, 8.57x previous step)
    6 steps has 408318 entries (55 percent, 3.32x previous step)
    7 steps has 187284 entries (25 percent, 0.46x previous step)
    8 steps has 1240 entries (0 percent, 0.01x previous step)

    Total: 735471 entries
    Average: 6.046301 moves
    """
    ...
    def state(self):
        ...
        wing_strs_to_stage = ('BL', 'BR', 'FL', 'FR')
        ...
        result = ''.join(['1' if state[x] == 'L' else '0' for x in edges_444])

        # Convert to hex
        return self.hex_format % int(result, 2)


class LookupTable444SecondFourEdgesStage(LookupTable):
    """
    lookup-table-4x4x4-step302-stage-second-four-edges.txt
    ======================================================
    1 steps has 5 entries (0 percent, 0.00x previous step)
    2 steps has 82 entries (0 percent, 16.40x previous step)
    3 steps has 1238 entries (0 percent, 15.10x previous step)
    ...
    def state(self):
        state = self.parent.state[:]
        wing_strs_to_stage = ('BD', 'BU', 'DF', 'FU')
        ...
        result = ''.join(['1' if state[x] == 'U' else '0' for x in edges_444])

        # Convert to hex
        return self.hex_format % int(result, 2)


class LookupTable444EdgesStage(LookupTableIDA):
    """
    lookup-table-4x4x4-step300-stage-edges.txt
    ==========================================
    1 steps has 7 entries (0 percent, 0.00x previous step)
    2 steps has 135 entries (0 percent, 19.29x previous step)
    3 steps has 2766 entries (0 percent, 20.49x previous step)
    4 steps has 56468 entries (5 percent, 20.42x previous step)
    5 steps has 1053666 entries (94 percent, 18.66x previous step)

    Total: 1113042 entries
    Average: 4.943908 moves
    """
    ...
    def __init__(self, parent):
        LookupTableIDA.__init__(
            ...
            parent,
            'lookup-table-4x4x4-step300-stage-edges.txt',
            'UUxxxxUUxxLLLLxxUULLLLUUxxLLLLxxUULLLLUUUUxxxxUU',
            moves_4x4x4,
            ...
            # prune tables
            (parent.lt_first_four_edges,
             parent.lt_second_four_edges),
    ...
    def state(self):
        ...
        U_wing_strs_to_stage = ('BD', 'BU', 'DF', 'FU')
        ...
        #edges = ''.join([state[x] for x in [2, 3, 5, 9, 8, 12, 14, 15, 21, 25, 24, 28, 53, 57, 56, 60, 82, 83, 85, 89, 88, 92, 94, 95]])
        edges = ''.join([state[x] for x in edges_444])
...
class RubiksCube444(RubiksCube):
    ...
    def lt_init(self):
        ...
        # stage UD centers and all edges for L4E
        self.lt_first_four_edges = LookupTable444FirstFourEdgesStage(self)
        self.lt_second_four_edges = LookupTable444SecondFourEdgesStage(self)

        self.lt_edges_stage = LookupTable444EdgesStage(self)
    ...
    def group_centers_guts(self, from_larger_cube=False):
        ...
        #self.lt_first_four_edges.solve()
        #self.lt_second_four_edges.solve()
        #self.print_cube()
