...
#define X_COST_MAX 11
#define T_COST_MAX 12
...
static float cost_to_goal_multiplier = 0.0f;
static move_type best_solution[MAX_IDA_THRESHOLD + 1];

/*
 * Indexed by the x-center and t-center ranked-table costs.
 * utils/build-555-centers-stage-matrix.py samples and rebuilds this matrix.
 */
static const unsigned char center_stage_costs_555[X_COST_MAX + 1][T_COST_MAX + 1] = {
    { 0,  1,  3,  4,  6,  7,  8, 10, 11, 13, 14, 15, 17},  // x cost 0
    { 1,  1,  4,  4,  6,  7,  8, 10, 11, 13, 14, 15, 17},  // x cost 1
    { 3,  3,  4,  4,  6,  7,  8, 10, 11, 13, 14, 15, 17},  // x cost 2
    { 4,  4,  4,  4,  6,  7,  8, 10, 11, 13, 14, 15, 17},  // x cost 3
    { 6,  6,  6,  6,  6,  7,  8, 11, 11, 13, 14, 15, 17},  // x cost 4
    { 7,  7,  7,  7,  7,  7,  8, 11, 12, 13, 14, 15, 17},  // x cost 5
    { 8,  8,  8,  8,  8,  8,  8, 11, 12, 13, 14, 15, 17},  // x cost 6
    {10, 10, 10, 10, 10, 10, 10, 11, 12, 13, 14, 15, 17},  // x cost 7
    {11, 11, 11, 11, 11, 11, 11, 11, 12, 13, 14, 15, 17},  // x cost 8
    {13, 13, 13, 13, 13, 13, 13, 13, 13, 13, 14, 15, 17},  // x cost 9
    {14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14, 15, 17},  // x cost 10
    {15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 17},  // x cost 11
};
...
static void usage(const char *program)
{
    printf(
        ...
        "[--multiplier F] [--serial-order] [--print-ranks] [--print-ida-summary]\n",
        program
    );
    printf(
        "  --multiplier F  scale max(x-cost, t-cost) by F instead of using the sampled\n"
        "                  center_stage_costs_555 matrix. F must be at least 1.0.\n"
    );
}
...
static unsigned char combined_cost(unsigned char x_cost, unsigned char t_cost)
{
    unsigned char admissible = x_cost > t_cost ? x_cost : t_cost;

    if (cost_to_goal_multiplier) {
        float scaled = roundf(admissible * cost_to_goal_multiplier);

        return scaled > MAX_IDA_THRESHOLD ? MAX_IDA_THRESHOLD + 1 : (unsigned char)scaled;
    }
    return center_stage_costs_555[
        x_cost > X_COST_MAX ? X_COST_MAX : x_cost
    ][
        t_cost > T_COST_MAX ? T_COST_MAX : t_cost
    ];
}
...
static struct heuristic_result heuristic(const char *cube)
{
    ...
    if (!x_encoded || !t_encoded) {
        ...
        result.cost = combined_cost(x_cost, t_cost);
    }
    return result;
}
...
// Returns UINT8_MAX when the state is absent from a table, else the combined cost.
static unsigned char cost_from_ranks(uint64_t x_rank, uint64_t t_rank)
{
    ...
    return combined_cost(x_encoded - 1, t_encoded - 1);
}
...
static void print_ida_summary(char cube[CUBE_ARRAY_SIZE_555], const move_type *solution)
{
    ...
    for (unsigned char idx = 0; idx <= solution_len; idx++) {
        ...
        unsigned char t_cost = t_encoded ? t_encoded - 1 : UINT8_MAX;
        unsigned char ctg =
            x_cost == UINT8_MAX || t_cost == UINT8_MAX ? UINT8_MAX : combined_cost(x_cost, t_cost);
...
)
{
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        cost = combined_cost(x_encoded - 1, t_encoded - 1);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strmatch(argv[i], "--kociemba") && i + 1 < argc) {
        ...
        } else if (strmatch(argv[i], "--multiplier") && i + 1 < argc) {
            cost_to_goal_multiplier = (float)atof(argv[++i]);
            ...
            print_ranks = 1;
        } else if (strmatch(argv[i], "--print-ida-summary")) {
            /* Kept for parity with the other matrix-sampling searchers; this
             * search always emits the summary after finding a solution. */
        } else {
            usage(argv[0]);
            return 2;
        }
    ...
    if (cost_to_goal_multiplier) {
        ...
        LOG("searching with the sampled x/t center cost matrix\n");
    }
    if (transposition_table) {
        LOG(
