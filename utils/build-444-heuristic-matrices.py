...
PHASE12_BINARY = "./ida_search_444_phase1_and_2"
PHASE34_BINARY = "./ida_search_444_phase3_and_4"
PHASE12_SAMPLES = Path("utils/444-phase12-samples.jsonl")
PHASE34_SAMPLES = Path("utils/444-phase34-samples.jsonl")
...
def phase12_command(cube, cores):
...
def apply_phase12(cube, solution):
    ...
    for mapping in mappings:
        if cube.highlow_edges_state(mapping) == PHASE12_HIGHLOW_TARGET_444:
            cube.edge_mapping = mapping or []
            return
    raise RuntimeError("phase 1+2 did not produce a high/low mapping")
...
def matrix_reports(records12, records34):
    ...
    matrix12, counts12 = fill_cost_matrix(
        samples12,
        (PHASE12_COST_MAX + 1,) * 3,
        lambda index: max(index),
    )
    print("\nphase 1+2 sample counts by populated cell:")
    print(f"  observations={len(samples12)} populated={sum(bool(value) for value in counts12.values())}")
    print(
        format_c_matrix_3d(
            "static const unsigned char phase12_cost_matrix_444"
            "[PHASE12_COST_MAX + 1][PHASE12_COST_MAX + 1][PHASE12_COST_MAX + 1] = {",
            matrix12,
            "CTR",
            "LR",
        )
    ...
    matrix34, counts34 = fill_cost_matrix(
        samples34,
        (PHASE34_EDGE_MAX + 1, PHASE34_CENTER_MAX + 1),
        lambda index: max(index),
    )
    print("phase 3+4 sample counts by populated cell:")
...
def main():
    ...
    records12 = load_records(PHASE12_SAMPLES)
    records34 = load_records(PHASE34_SAMPLES)
    if not args.report_only:
        print(
            f"cubes={args.count} timeout={args.timeout:.0f}s cores={args.cores} "
            f"phase12_done={len(records12)} phase34_done={len(records34)}",
            flush=True,
        )
        with PHASE12_SAMPLES.open("a") as out12, PHASE34_SAMPLES.open("a") as out34:
            for index in range(args.count):
                ...
                if sample_id not in records12 or records12[sample_id]["timeout"]:
                    command12 = phase12_command(cube, args.cores)
                    result12 = run_timed_solve(command12, args.timeout)
                    output12 = result12.pop("output")
                    path12 = parse_phase12_path(output12) if result12["ok"] else []
                ...
                if not result12["ok"] or not solution12:
                    print(
                        f"sample={sample_id:04d} phase12 " f"{'TIMEOUT' if result12['timeout'] else 'FAIL'}",
                        flush=True,
                    )
                    continue

                apply_phase12(cube, solution12)
                if sample_id not in records34:
                    command34 = phase34_command(cube, args.cores)
                    result34 = run_timed_solve(command34, args.timeout)
                    output34 = result34.pop("output")
                    path34 = parse_phase34_path(output34) if result34["ok"] else []
